...
export default function Sidebar({ onOpenMemberModal, onOpenTeamModal }) {
  ...
  return (
    <div className="w-64 h-screen bg-[#080808]/95 backdrop-blur-xl border-r border-white/10 flex flex-col justify-between shrink-0 select-none">
      <div>
        ...
        <div className="p-4 border-b border-white/10 relative">
          ...
          <div className="mt-3 relative">
            <button
              ...
              className="w-full bg-[#121212] hover:bg-[#181818] text-slate-200 px-3 py-2 rounded-xl flex items-center justify-between text-xs font-semibold border border-white/10 transition-all"
            >
            ...
            {showTeamDropdown && (
              <div className="absolute top-full left-0 w-full mt-1 bg-[#0D0D0D] border border-white/10 rounded-xl shadow-2xl z-50 p-1">
                ...
                <button
                  ...
                  className="w-full text-left px-3 py-2 text-xs text-emerald-400 hover:bg-emerald-950/40 rounded-lg flex items-center gap-1.5 font-medium border-t border-white/5 mt-1"
                >
          ...
          <button
            ...
            className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-xs font-medium transition-all ${
              activeTab === 'chat' && !activeDmUser
                ? 'bg-[#181818] text-emerald-400 border border-emerald-500/30 shadow-inner'
                : 'text-slate-400 hover:bg-[#121212] hover:text-slate-200'
            }`}
          >
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-2.5 px-2.5 py-1.5 rounded-lg text-xs transition-colors ${
                      isSelected
                        ? 'bg-[#181818] text-emerald-300 font-medium border border-emerald-500/20'
                        : 'text-slate-400 hover:text-slate-200 hover:bg-[#121212]'
                    }`}
                  >
                    <div
      ...
      <div className="p-3 border-t border-white/10 bg-[#0A0A0A] flex items-center justify-between">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="truncate">
            ...
            <p className="text-[10px] text-emerald-400 truncate">@{user?.username}</p>
          </div>
        </div>
